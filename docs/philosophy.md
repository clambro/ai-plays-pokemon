...
Each iteration produces a short record of what the model intended to do and what actually happened. The most recent hundred or so records are included in the prompt verbatim. Once this exact tail gets too long, the oldest twenty records are compressed into a single summary. When a third twenty-iteration summary appears, the oldest two become a forty-iteration summary while the newest remains intact. The same rule applies at each larger size, keeping recent history at every level from being compressed all at once.
