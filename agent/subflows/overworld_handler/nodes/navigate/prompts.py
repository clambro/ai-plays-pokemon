...
The coordinates in the format (row, col, tile type) that are accessible from your current position are as follows:
<accessible_coords>
{accessible_coords}
</accessible_coords>

The following coordinates (a subset of the accessible coordinates provided above) are adjacent to unseen territory on the current map. They are therefore top candidates for exploration. If the section below is empty, then you have already explored all of the accessible tiles on the current map. Navigating towards any of these exploration candidates is the most efficient way to explore the current map.
...
- If the last memory asks you to navigate to a specific tile type (e.g. tall grass or water), you must move to an accessible coordinate of that tile type. If you are specifically trying to find wild Pokemon, move to tiles that are at least five tiles away from you. The more grass/cave/water tiles you walk through, the more likely you are to find wild Pokemon.
