...
class NavigationService:
    ...
    async def _handle_pikachu(self, button: Button) -> bool:
        ...
        if not needs_pikachu_turn:
            return True
        control_result, _ = await self._press_and_record_boundary(button, game_state)
    ...
    async def _press_navigation_step(
        ...
        *,
        ...
        result, observed_state = await self._press_and_record_boundary(
            button,
            game_state,
        ...
        if turned_without_moving or pikachu_was_ahead:
            result, _ = await self._press_and_record_boundary(
                button,
                observed_state,
                observe_steps=observe_steps,
            )
    ...
    async def _handle_hm_use(
        ...
        if game_state.player.direction != _BUTTON_DIRECTIONS[button]:
            result, _ = await self._press_and_record_boundary(button, game_state)
            if result.boundary != ControlBoundary.OVERWORLD_READY:
                return "", result.boundary
