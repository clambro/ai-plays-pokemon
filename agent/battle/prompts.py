...
def _format_battle_guidance(game_state: GameState) -> str:
    ...
    guidance = "Using a move or voluntarily switching Pokemon consumes the turn. Switching gives the opponent an opportunity to attack the Pokemon switched in. Consider the damage the incoming Pokemon will take, not just the active Pokemon's low HP; letting the active Pokemon faint can sometimes provide a safer entry for its replacement. Experience is granted only to Pokemon used in the battle, provided they have not fainted and are not at the level cap."
