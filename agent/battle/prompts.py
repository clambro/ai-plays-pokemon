...
def _format_battle_guidance(game_state: GameState) -> str:
    ...
    if battle_type == BattleType.WILD:
        guidance += " If you are not deliberately training and do not intend to catch this Pokemon, running is the default. A favorable matchup, easy victory, or generally useful experience is not by itself a reason to fight. An unsuccessful capture attempt or failed escape also gives the opponent an opportunity to attack."
    return guidance
