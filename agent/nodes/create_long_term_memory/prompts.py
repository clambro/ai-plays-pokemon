"""LLM prompts for create long term memory in the top-level agent graph."""
...
Note that long-term memory titles are unique. You cannot re-create or edit an existing title here. You will have the chance to do that elsewhere. The purpose of this interaction is to create a totally new memory document if you feel that you have learned something important that is not already represented in your long-term memory.
...
- Do not create duplicate or near-duplicate memories. If you already have a memory with a similar title to one that you might create here, do not create a new one. You will have the chance to edit existing memories elsewhere.
- Titles must be unique.
- Long term memory objects must be concise and to the point. A couple paragraphs max.
- Long term memory objects must not include mundane information like wild Pokemon that were defeated or individual moves that were used. A good rule of thumb is that everything in your long term memory should still be relevant a thousand iterations from now. If it won't be, it does not need to be in there.
